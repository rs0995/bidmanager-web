...
class DurableJobRuntimeTests(unittest.TestCase):
    ...
    def _run(self, root: Path, source: str, timeout: int = 30) -> subprocess.CompletedProcess:
        env = os.environ.copy()
        for key in (
            "K_SERVICE",
            "DATABASE_URL",
    ...
    def test_running_job_is_requeued_after_a_worker_restart(self):
        with tempfile.TemporaryDirectory(prefix="bidmanager-restart-resume-") as tmp:
            root = self._isolated_server_copy(Path(tmp))
            first = self._run(
                root,
                """
                ...
                def running(job_id, action, attempt_count, cancel=False):
                    now = time.time()
                    job = {
                        'status':'running', 'action':action, 'payload':{'website_id':1},
                        'result':None, 'error':'', 'logs':[], 'progress':10,
                        'created_at':now-30, 'started_at':now-25, 'finished_at':None,
                        'heartbeat_at':now-25, 'cancel_requested':cancel,
                        'attempt_count':attempt_count, 'worker_id':'dead-worker',
                        'website_id':1, 'tender_id':None, 'mode':None,
                    }
                    with api._job_lock:
                        api._jobs[job_id] = job
                ...
                api.core.ScraperBackend.set_setting('retry_attempts', '3')
                running('job_resume_me', 'archive_completed_tenders', 1)
                running('job_gave_up', 'archive_completed_tenders', 3)
                running('job_was_cancelled', 'archive_completed_tenders', 1, cancel=True)
                api._job_executor.shutdown(wait=True, cancel_futures=True)
                """,
            )
            self.assertEqual(first.returncode, 0, first.stdout + first.stderr)
            ...
            second = self._run(
                ...
                want = ('completed', 'failed', 'cancelled')
                deadline = time.time() + 20
                while time.time() < deadline:
                    with api._job_lock:
                        sts = [api._jobs.get(j, {}).get('status')
                               for j in ('job_resume_me', 'job_gave_up', 'job_was_cancelled')]
                    if all(s in want for s in sts):
                        break
                    time.sleep(0.05)

                with api.get_db() as conn:
                    rows = {r[0]: (r[1], r[2]) for r in conn.execute(
                        "SELECT id,status,error FROM background_jobs WHERE id IN "
                        "('job_resume_me','job_gave_up','job_was_cancelled')"
                    ).fetchall()}
                assert rows['job_resume_me'][0] == 'completed', rows['job_resume_me']
                assert rows['job_gave_up'][0] == 'failed', rows['job_gave_up']
                assert 'stopped 3 times' in rows['job_gave_up'][1], rows['job_gave_up']
                assert rows['job_was_cancelled'][0] == 'cancelled', rows['job_was_cancelled']
