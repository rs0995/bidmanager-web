...
def new_chromium_options():
    ...
    if not binary:
        for candidate in (shutil.which("chromium"), shutil.which("chromium-browser"), "/usr/bin/chromium"):
...
def new_browser_driver():
    if configured_browser() == "chromium":
        ...
        # version-matched /usr/bin/chromedriver); otherwise let Selenium Manager
        # resolve one, as on a dev machine.
        driver_path = (
            os.getenv("BIDMANAGER_CHROMEDRIVER", "").strip()
...
def _init_db_schema(conn):
    ...
    try:
        # Reversibly-encrypted password copy for the admin console (see security.py).
        c.execute("ALTER TABLE client_users ADD COLUMN password_enc TEXT")
    except sqlite3.OperationalError:
        pass
...
class ScraperBackend:
    ...
    @staticmethod
    def validate_captcha_ai_config(config):
        """Verify a Gemini CAPTCHA config is usable. Raises ValueError with a
        user-facing message when the key is rejected or the model is unavailable.
        Called from the settings save path so a bad key is caught at entry time,
        not mid-download. Only Gemini is checked; other providers are a no-op."""
        provider = str(config.get("provider") or "").strip().lower()
        if provider != "gemini":
            return
        api_key = str(config.get("api_key") or "").strip()
        model = str(config.get("model") or "").strip().split("/")[-1]
        if not api_key:
            raise ValueError("Enter a Gemini API key.")
        if not model:
            raise ValueError("Enter a Gemini model, e.g. gemini-2.5-flash.")
        if not ensure_scraper_dependencies() or genai is None:
            raise ValueError("CAPTCHA AI libraries are unavailable on this server.")
        try:
            genai.configure(api_key=api_key)
            names = {
                str(getattr(m, "name", "")).split("/")[-1]
            ...
            if any(s in text for s in (
                ...
                "permission_denied", "unauthenticated", " 400", " 401", " 403",
            )):
                raise ValueError("Google rejected this Gemini API key.")
            raise ValueError(
                "Could not reach Gemini to verify the key — check connectivity and try again."
            )
        if names and model not in names:
            hint = ", ".join(sorted(n for n in names if n.startswith("gemini"))[:6]) or "gemini-3.6-flash"
            raise ValueError(f"Model '{model}' isn't available for this key. Try one of: {hint}")
        # list_models() also returns models that 404 on generate ("no longer
        # available to new users"), so probe the model with a tiny request.
        try:
            genai.GenerativeModel(model).generate_content("ok")
        except Exception as e:
            text = str(e).lower()
            if "429" in text or "quota" in text or "rate limit" in text:
                return  # real model, just rate-limited right now
            if any(s in text for s in ("api_key_invalid", "api key not valid", "unauthenticated", " 401", " 403")):
                raise ValueError("Google rejected this Gemini API key.")
            if "404" in text or "not found" in text or "no longer available" in text or "not supported" in text:
                short = str(e).split("Please update")[0].strip().rstrip(".")
                raise ValueError(f"Model '{model}' can't be used with this key ({short}). Try gemini-3.6-flash.")
            raise ValueError(f"Model '{model}' could not be verified: {str(e)[:160]}")
                            ...
                            if not dead:
                                try:
                                    ...
                                    if _conn_is_dead(e):
                                        ...
                                        raise
                            if not dead:
                                db_errors += page_db_errors
                                break
                            if _save_try == 1:
                                log_to_gui(
                                    f"Database connection dropped mid-scrape; reconnecting and "
                                    f"re-saving this page for {org_name}."
                                )
                                _reconnect()
                                continue
                            db_errors += len(tenders_to_save)
                            log_to_gui(
                                f"Page save failed for {org_name} after reconnect: the database "
                                f"connection is closed. {len(tenders_to_save)} tender(s) not saved."
                            )
                ...
                try:
                    for _sched_try in (1, 2):
                        try:
                            ...
                            if _sched_try == 1 and _conn_is_dead(e):
                                _reconnect()
                                continue
                            raise
                except Exception as e:
    ...
    def download_single_tender_logic(tender_db_id, mode):
        ...
        if int(is_archived or 0) != 0:
            ...
            return False
        result = ScraperBackend.download_tenders_logic(
            website_id, target_db_ids=[target_id], forced_mode=mode_txt
        )
        return result is not False

    @staticmethod
    def archive_completed_tenders_logic(website_id, emit_log=True):
        conn = sqlite3.connect(DB_FILE)
        c = conn.cursor()
