...
function createApi(base, adminKey) {
  return {
    ...
    async downloadTenders(websiteId, tenderIds, all = false, mode = 'full') {
      return apiFetch(base, `/v1/websites/${Number(websiteId)}/tenders/download-batch`, {
        method: 'POST', adminKey,
        body: { tender_ids: tenderIds, all_tenders: all, mode },
      });
    },
    async scheduleTenders(websiteId, tenderIds, intervalMinutes, enabled = true) {
      return apiFetch(base, `/v1/websites/${Number(websiteId)}/tenders/schedule-batch`, {
        method: 'PUT', adminKey,
        body: { tender_ids: tenderIds, interval_minutes: intervalMinutes, enabled },
      });
    },
...
function NumIn({ value, onChange, suffix, w = 88, step = 1, min }) {
  return (
        ...
        <div>
          ...
          {group('Captcha solving', <>
            ...
            {draft.captcha_ai_provider !== 'manual' && <>
              <Field label="Model ID" hint="Enter the exact vision-capable model identifier supplied by the provider" dirty={isDirty('captcha_ai_model')}>
                <TextIn value={draft.captcha_ai_model || ''} onChange={(v) => set('captcha_ai_model', v)} w={240} placeholder="Provider model ID" />
              </Field>
...
function JobsPanel({ toast, base, adminKey }) {
  ...
  const startScrape = async () => {
    ...
    try {
      ...
      toast(err.message || 'Could not start scraper');
    }
    setStarting(false);
  };
      ...
      <Card style={{ marginBottom: 10 }}>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div style={{ fontSize: 12, color: c.ink60 }}>Refresh organizations, then fetch tenders for the selected portal.</div>
          </div>
          <div className="flex items-center gap-2">
            <Select
              value={websiteId}
              ...
              w={210}
              options={websites.map((website) => ({ value: String(website.id), label: website.name }))}
            />
            <Btn variant="primary" icon={Play} busy={starting} disabled={!websiteId} onClick={startScrape}>Run scraper</Btn>
          </div>
        </div>
      </Card>
      <StatStrip items={[
...
function ScraperPanel({ toast, base, adminKey }) {
  ...
  const [scheduleMinutes, setScheduleMinutes] = useState(180);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [orgRows, tenderRows] = await Promise.all([
        api.organizations(websiteId), api.tenders(websiteId),
      ]);
      setOrganizations(Array.isArray(orgRows) ? orgRows : []);
  ...
  const queueScrape = async (all) => {
    const currentOrgIds = organizations.filter((org) => selectedOrgs.has(org.id)).map((org) => org.id);
    if (!all && currentOrgIds.length === 0) return;
    setBusy(all ? 'scrape-all' : 'scrape-selected');
    try {
      const job = await api.scrapeOrganizations(websiteId, currentOrgIds, all);
      toast(`Scrape job ${job.job_id || job.id || ''} queued`);
    } catch (err) { toast(err.message || 'Could not queue scrape'); }
    setBusy('');
  };

  const queueDownload = async (all) => {
    const currentTenderIds = tenders.filter((tender) => selectedTenders.has(tender.id)).map((tender) => tender.id);
    if (!all && currentTenderIds.length === 0) return;
    setBusy(all ? 'download-all' : 'download-selected');
    try {
      const job = await api.downloadTenders(websiteId, currentTenderIds, all, downloadMode);
  ...
  const saveSchedule = async (enabled) => {
    ...
    setBusy(enabled ? 'schedule' : 'unschedule');
    try {
      const result = await api.scheduleTenders(
        websiteId, currentTenderIds, Number(scheduleMinutes) || 0, enabled,
      );
      toast(`${result.updated || currentTenderIds.length} tender schedule${enabled ? 's saved' : 's disabled'}`);
      await loadData();
    } catch (err) { toast(err.message || 'Could not update tender schedules'); }
    setBusy('');
  };
  ...
  const saveNamedJob = async () => {
    ...
    if ((jobType === 'scrape' || jobType === 'both') && selectedOrgs.size === 0) return toast('Select at least one organization');
    if ((jobType === 'download' || jobType === 'both') && selectedTenders.size === 0) return toast('Select at least one tender');
    ...
    if (savedJobScheduleMode === 'interval' && Number(scheduleMinutes) <= 0) return toast('Enter a positive repeat interval');
    setBusy('save-custom');
    try {
      const saved = await api.saveCustomJob({
        ...
        download_mode: downloadMode, schedule_enabled: savedJobScheduleMode !== 'manual',
        schedule_mode: savedJobScheduleMode,
        interval_minutes: savedJobScheduleMode === 'interval' ? Number(scheduleMinutes) || 0 : 0,
  ...
  const editSavedJob = (job) => {
    ...
    setJobType(job.job_type || 'scrape');
    setDownloadMode(job.download_mode || 'full');
    ...
    setScheduleMinutes(job.interval_minutes || 180);
  };
  ...
  const selectionReady = jobType === 'scrape'
    ? selectedOrgs.size > 0
    : jobType === 'download'
      ? selectedTenders.size > 0
      : selectedOrgs.size > 0 && selectedTenders.size > 0;
  const savedJobScheduleLabel = (job) => {
    ...
    if (mode === 'interval' && job.schedule_enabled) return `every ${job.interval_minutes} min`;
    return 'run manually';
  };
    ...
    <Panel
      ...
      right={
        <div className="flex items-center gap-2">
          ...
          <Btn size="sm" icon={RefreshCw} busy={loading} disabled={!websiteId} onClick={loadData}>Refresh</Btn>
        </div>
      }
    >
      <Card pad={false} style={{ position: 'sticky', top: 52, zIndex: 5, marginBottom: 10, boxShadow: '0 6px 18px rgba(21,32,42,0.10)' }}>
        <div className="flex flex-wrap items-center justify-between gap-3 px-3 py-3" style={{ borderBottom: `1px solid ${c.rule}` }}>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {savedJobScheduleMode === 'interval' && <NumIn value={scheduleMinutes} onChange={setScheduleMinutes} min={1} suffix="min" w={70} />}
          ...
          <div className="overflow-x-auto">
            <table className="w-full" style={{ borderCollapse: 'collapse' }}>
              ...
              <tbody>{savedJobs.map((job) => <tr key={job.id} style={{ borderBottom: `1px solid ${c.ruleSoft}` }}>
                ...
                <td className="px-3 py-2"><Pill state={job.job_type === 'scrape' ? 'info' : job.job_type === 'both' ? 'warn' : 'ok'}>{job.job_type}</Pill></td>
      ...
      <Card pad={false} style={{ marginBottom: 10 }}>
        <div className="flex flex-wrap items-center justify-between gap-3 px-3 py-3" style={{ borderBottom: `1px solid ${c.rule}` }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: c.ink60 }}>Select organizations to scrape, or queue every organization on this portal.</div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <TextIn value={orgSearch} onChange={setOrgSearch} w={220} placeholder="Search organizations" />
            <Btn size="sm" icon={Play} busy={busy === 'scrape-selected'} disabled={currentSelectedOrgCount === 0} onClick={() => queueScrape(false)}>Run once: selected ({currentSelectedOrgCount})</Btn>
            <Btn size="sm" variant="primary" icon={Play} busy={busy === 'scrape-all'} disabled={organizations.length === 0} onClick={() => queueScrape(true)}>Run once: all</Btn>
          </div>
        </div>
        <div className="overflow-auto" style={{ maxHeight: 310 }}>
          <table className="w-full" style={{ borderCollapse: 'collapse' }}>
            <thead style={{ position: 'sticky', top: 0, zIndex: 1 }}><tr style={{ background: c.paper }}>
              <th className="px-3 py-2 text-left" style={{ width: 36, borderBottom: `1px solid ${c.rule}` }}><input type="checkbox" aria-label="Select shown organizations" checked={shownOrgs.length > 0 && shownOrgs.every((org) => selectedOrgs.has(org.id))} onChange={(e) => setSelectedOrgs((current) => { const next = new Set(current); shownOrgs.forEach((org) => e.target.checked ? next.add(org.id) : next.delete(org.id)); return next; })} style={{ accentColor: c.indigo }} /></th>
            ...
            <tbody>{shownOrgs.map((org) => <tr key={org.id} style={{ borderBottom: `1px solid ${c.ruleSoft}` }}>
              <td className="px-3 py-2"><input type="checkbox" checked={selectedOrgs.has(org.id)} onChange={() => toggle(setSelectedOrgs, org.id)} style={{ accentColor: c.indigo }} /></td>
      ...
      <Card pad={false}>
        <div className="flex flex-wrap items-center justify-between gap-3 px-3 py-3" style={{ borderBottom: `1px solid ${c.rule}` }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: c.ink60 }}>Full downloads save the initial package once; Updates checks only later pre-bid and corrigendum/addendum files.</div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <TextIn value={tenderSearch} onChange={setTenderSearch} w={210} placeholder="Search tenders" />
            <Select value={downloadMode} onChange={setDownloadMode} w={115} options={[{ value: 'full', label: 'Full' }, { value: 'update', label: 'Updates' }]} />
            <Btn size="sm" icon={Download} busy={busy === 'download-selected'} disabled={currentSelectedTenderCount === 0} onClick={() => queueDownload(false)}>Run once: selected ({currentSelectedTenderCount})</Btn>
            <Btn size="sm" variant="primary" icon={Download} busy={busy === 'download-all'} disabled={tenders.length === 0} onClick={() => queueDownload(true)}>Run once: all</Btn>
          </div>
        </div>
        <div className="flex flex-wrap items-center justify-between gap-3 px-3 py-2" style={{ background: c.paper, borderBottom: `1px solid ${c.rule}` }}>
          <span style={{ fontSize: 11.5, color: c.ink60 }}>Interval for selected tenders</span>
          <div className="flex items-center gap-2">
            <NumIn value={scheduleMinutes} onChange={setScheduleMinutes} min={1} suffix="min" w={74} />
            <Btn size="sm" icon={Timer} busy={busy === 'schedule'} disabled={currentSelectedTenderCount === 0 || Number(scheduleMinutes) <= 0} onClick={() => saveSchedule(true)}>Save schedule</Btn>
            <Btn size="sm" variant="ghost" busy={busy === 'unschedule'} disabled={currentSelectedTenderCount === 0} onClick={() => saveSchedule(false)}>Disable schedule</Btn>
          </div>
        </div>
