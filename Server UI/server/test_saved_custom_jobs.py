...
class SavedCustomJobTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory(prefix="bidmanager-saved-job-") as tmp:
            ...
            result = subprocess.run(
                [sys.executable, "-c", textwrap.dedent("""
                    ...
                    original_within_window = api._within_scrape_window
                    api._saved_job_scheduler_enabled = lambda: True
                    api._claim_scheduler_lease = lambda *args, **kwargs: True
                    api._within_scrape_window = lambda *args, **kwargs: True
                    ...
                    api._within_scrape_window = original_within_window
                    completed_once = next(
                        job for job in api.list_saved_custom_jobs('Alice', None)['jobs'] if job['id'] == once['id']
                    )
        ...
        with tempfile.TemporaryDirectory(prefix="bidmanager-consolidated-") as tmp:
            ...
            result = subprocess.run(
                [sys.executable, "-c", textwrap.dedent("""
                    ...
                    api._saved_job_scheduler_enabled = lambda: True
                    api._within_scrape_window = lambda *a, **k: True
    ...
    def test_all_organizations_wildcard_does_not_swallow_an_explicitly_due_org(self):
        # Regression test for a bug caught live: an "all organizations" saved
        # job only actually scrapes whatever the admin has is_selected=1 —
        # if it silently superseded an explicitly-due org that isn't
        # is_selected, that org would never actually get scraped even though
        # its contributor was credited as served.
        with tempfile.TemporaryDirectory(prefix="bidmanager-wildcard-") as tmp:
            ...
            result = subprocess.run(
                [sys.executable, "-c", textwrap.dedent("""
                    ...
                    assert len(captured) == 1, captured
                    action, payload = captured[0]
                    # Must resolve to an explicit selected-org call that
                    # includes Org A — NOT a bare "fetch_tenders" wildcard
                    # that would silently skip Org A since it isn't is_selected.
                    assert action == 'fetch_tenders_selected', action
                    assert payload['org_ids'] == [org_a], payload['org_ids']

                    api.delete_saved_custom_job(wildcard['id'], 'Carol', None)
                    api.delete_saved_custom_job(explicit['id'], 'Dave', None)
                """)],
                cwd=root,
                env=env,
                text=True,
    ...
    def test_all_organizations_covers_every_org_not_just_is_selected(self):
        # Regression test: an "entire website" saved job must cover literally
        # every organization for the website, on both the manual-run and the
        # scheduled/consolidated path — it must never depend on
        # organizations.is_selected (an unrelated, client-facing watchlist
        # flag), even when it's the only saved job due this tick.
        with tempfile.TemporaryDirectory(prefix="bidmanager-all-orgs-") as tmp:
            ...
            result = subprocess.run(
                [sys.executable, "-c", textwrap.dedent("""
                    ...
                    action, payload = captured[0]
                    assert action == 'fetch_tenders_selected', action
                    assert sorted(payload['org_ids']) == org_ids, (payload['org_ids'], org_ids)
