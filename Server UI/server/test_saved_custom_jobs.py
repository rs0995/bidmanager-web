...
class SavedCustomJobTests(unittest.TestCase):
    ...
    def test_whole_website_job_with_all_orgs_selected_refreshes_then_scrapes_all(self):
        # "Whole website" toggle + org "select all" (non-empty org_ids) =>
        # the fetch_organisations job carries followup_action='fetch_tenders'
        # and followup_all_organizations=True so, after the refresh, tenders
        # are scraped for every org that exists post-refresh. True on both the
        # manual-run and the scheduled/consolidated path.
        with tempfile.TemporaryDirectory(prefix="bidmanager-refresh-scrape-") as tmp:
            root = Path(tmp)
            for source in SOURCE_DIR.glob("*.py"):
                if not source.name.startswith("test_"):
                    shutil.copy2(source, root / source.name)
            ...
            result = subprocess.run(
                [sys.executable, "-c", textwrap.dedent("""
                    ...
                    # Manual run.
                    manual = api.create_saved_custom_job(api.SavedCustomJobRequest(
                        owner_name='Gwen', name='Whole-site refresh+scrape (manual)', website_id=1,
                        job_type='scrape', all_organizations=True, org_ids=org_ids,
                        schedule_enabled=False,
                    ), None)
                    api.run_saved_custom_job(manual['id'], 'Gwen', None)
                    assert len(captured) == 1, captured
                    action, payload = captured[0]
                    assert action == 'fetch_organisations', action
                    assert payload['website_id'] == 1, payload
                    assert payload.get('followup_action') == 'fetch_tenders', payload
                    assert payload.get('followup_all_organizations') is True, payload
                    assert 'org_ids' not in payload, payload
                    api.delete_saved_custom_job(manual['id'], 'Gwen', None)
                    captured.clear()

                    # Scheduled/consolidated run.
                    scheduled = api.create_saved_custom_job(api.SavedCustomJobRequest(
                        owner_name='Hank', name='Whole-site refresh+scrape (scheduled)', website_id=1,
                        job_type='scrape', all_organizations=True, org_ids=org_ids,
                        schedule_enabled=True, schedule_mode='interval', interval_minutes=60,
                    ), None)
                    with api.get_db() as conn:
                        conn.execute('UPDATE saved_custom_jobs SET next_run_at=? WHERE id=?', (time.time() - 1, scheduled['id']))
                        conn.commit()
