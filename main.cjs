...
const BACKEND_PORT = Number(process.env.BIDMANAGER_PORT || 8000);
...
ipcMain.handle("desktop:open-project-window", async (_event, projectId) => {
  ...
  try {
    ...
    const url = isDev
      ? `${DEV_FRONTEND_URL}?projectId=${pid}`
      : `${BACKEND_URL}?projectId=${pid}`;
    ...
    const tryOnce = () => {
      const req = http.get(`${url}/health`, (res) => {
        ...
        if (res.statusCode && res.statusCode >= 200 && res.statusCode < 500) {
          ...
          reject(new Error(`Backend health check failed with ${res.statusCode}`));
        } else {
...
function startBackend() {
  const env = { ...process.env, BIDMANAGER_PORT: String(BACKEND_PORT) };
  const spawnOpts = {
    env,
    windowsHide: true,
    stdio: "ignore",
  ...
  if (isDev) {
    ...
    backendProc = spawn("python", [scriptPath], spawnOpts);
    return;
  }
