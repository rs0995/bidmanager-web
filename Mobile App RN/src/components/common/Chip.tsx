...
export function Chip({ active, children, onPress }: ChipProps) {
  ...
  return (
    <Pressable
      onPress={onPress}
      className={cn(
        // Outline style: always a white/surface-0 fill, only the border
        // (and label color, above) switches to accent when active.
        "flex-row items-center gap-1 px-3.5 py-[7px] rounded-full border border-border bg-surface-0",
