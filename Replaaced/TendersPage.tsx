...
export default function TendersPage() {
  ...
  const [lastNonEmptyOrgs, setLastNonEmptyOrgs] = useState<Organization[]>([]);
  ...
  useEffect(() => {
    if (allOrgs && allOrgs.length > 0) {
      setLastNonEmptyOrgs(allOrgs);
    }
  }, [allOrgs]);

  const orgRows = useMemo(() => {
    const sourceOrgs = (allOrgs && allOrgs.length > 0) ? allOrgs : lastNonEmptyOrgs;
    let list = sourceOrgs || [];
    if (selectedWebsiteId) list = list.filter((o) => o.website_id === selectedWebsiteId);
    return cSearch(list, search);
  }, [allOrgs, lastNonEmptyOrgs, selectedWebsiteId, search]);
      ...
      for (const t of target) {
        ...
        if (!ensuredFolderPath) {
          try {
            ...
            const target = `${rootFolder}${rootFolder.endsWith('\\') || rootFolder.endsWith('/') ? '' : '/'}${leaf}`;
        ...
        // Best-effort metadata/materialization pass.
        // Do not fail project creation if metadata write is unavailable.
        try {
          const ensuredFinal = await api.ensureProjectFolder(Number(createdProject.id));
          ensuredFolderPath = String(ensuredFinal?.folder_path || ensuredFolderPath || '').trim();
        } catch {
          // continue; project row may still be valid even on old backend builds
        }
        if (!ensuredFolderPath) {
        ...
        try {
          ...
          if (desktop?.pathExists) {
            const m1 = await desktop.pathExists(`${ensuredFolderPath}${ensuredFolderPath.endsWith('\\') || ensuredFolderPath.endsWith('/') ? '' : '\\'}project_info.json`);
            const m2 = await desktop.pathExists(`${ensuredFolderPath}${ensuredFolderPath.endsWith('\\') || ensuredFolderPath.endsWith('/') ? '' : '\\'}.bidmanager_project.json`);
            const ok1 = Boolean(m1?.ok && m1?.exists);
            const ok2 = Boolean(m2?.ok && m2?.exists);
            if (!ok1 && !ok2) {
              setLogs((prev) => [...prev, `Warning: metadata file missing for project ${createdProject.id} (${sourceTenderId || title}).`]);
            }
          }
        } catch {
          // metadata check is best-effort only
        }
  ...
  const runJob = useCallback(async (start: (site: number) => Promise<{ job_id: string }>, targetSites: number[]) => {
      ...
      try {
        ...
        while (!done) {
          ...
          try {
            ...
            if (cp && !pendingCaptcha) setPendingCaptcha(cp);
          } catch {
            // ignore polling errors
          }
          const newLogs = Array.isArray(job.logs) ? job.logs : [];
          if (newLogs.length > seen) {
            const fresh = newLogs.slice(seen).map((x) => String(x));
            setLogs((prev) => [...prev, ...fresh]);
            seen = newLogs.length;
          }
          if (job.status === 'completed') done = true;
          if (job.status === 'failed') {
            if (job.error) setLogs((prev) => [...prev, String(job.error)]);
            done = true;
          }
        }
      } catch (err) {
    ...
    invalidate();
    setJobRunning(false);
  }, [jobRunning, pendingCaptcha, setTendersViewTab]);
  ...
  const fetchHighlightedTenders = useCallback(async () => {
    ...
    invalidate();
    setJobRunning(false);
  }, [chosenOrgGroups, jobRunning, pendingCaptcha, setTendersViewTab]);
  ...
  const onOrgKeyDown = (evt: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (key === 'ArrowDown' || key === 'ArrowUp' || key === 'Home' || key === 'End') {
      ...
      if (key === 'ArrowDown') nextIndex = Math.min(sortedOrgs.length - 1, nextIndex + 1);
  ...
  const onTenderKeyDown = (evt: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (key === 'ArrowDown' || key === 'ArrowUp' || key === 'Home' || key === 'End') {
      evt.preventDefault();
      let nextIndex = currentIndex < 0 ? 0 : currentIndex;
      if (key === 'ArrowDown') nextIndex = Math.min(activeTenders.length - 1, nextIndex + 1);
      if (key === 'ArrowUp') nextIndex = Math.max(0, nextIndex - 1);
      if (key === 'Home') nextIndex = 0;
      ...
      <div className="flex-1 overflow-auto" style={{ marginRight: '1px' }}>
                    ...
                    return (
                      ...
                      >
                        {visibleTenderCols.map((c) => {
                          ...
                          if (c.key === 'title') return <td key={c.key}><p className="overflow-hidden text-sm font-medium text-[var(--text)]" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{t.title}</p></td>;
        ...
        {tab === 'logs' && (
          <div className="min-h-full space-y-0.5 bg-[var(--surface-0)] p-4 font-mono text-xs">
            {logs.map((line, i) => (
              <p key={`${i}-${line.slice(0, 12)}`} className={cn('leading-relaxed whitespace-pre-wrap', line.includes('Error') || line.includes('Failed') ? 'text-rose-400' : line.includes('Done') ? 'text-emerald-400' : 'text-[var(--text-muted)]')}>{line}</p>
            ))}
          </div>
        )}
      </div>

      {showManage && <ManageWebsitesDialog websites={websites || []} onClose={() => { setShowManage(false); qc.invalidateQueries({ queryKey: ['websites'] }); }} />}
      {showClearDialog && (
        <ClearDataDialog
...
function ManageWebsitesDialog({ websites, onClose }: { websites: Website[]; onClose: () => void }) {
  const [name, setName] = useState('');
  const [url, setUrl] = useState('');
  const [statusUrl, setStatusUrl] = useState('');

  const addWebsite = useMutation({
    mutationFn: () => api.createWebsite({ name, url, status_url: statusUrl }),
    onSuccess: () => {
      setName('');
      setUrl('');
      setStatusUrl('');
